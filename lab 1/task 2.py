...
# Для простоты выберем только один признак, например 'bmi' (индекс массы тела)
# Найдём индекс этого признака
bmi_index = feature_names.index('bmi')

# Выберем только этот столбец
...
# 4. Реализация линейной регрессии вручную (метод наименьших квадратов)
# Формулы: b = cov(X, y) / var(X), a = mean(y) - b * mean(X)
...
# 5. Визуализация данных и прямых регрессии
...
# 6. Таблица предсказаний
# Для примера выведем первые 10 строк
